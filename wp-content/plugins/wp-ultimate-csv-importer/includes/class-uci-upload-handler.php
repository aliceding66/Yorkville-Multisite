...
if(!class_exists('SmackUCIUploadHandler')) {
	class SmackUCIUploadHandler {
		...
		function __construct($options = null, $initialize = true, $error_messages = null) {
			...
			$this->options = array('script_url' => $this->get_full_url() . '/', 'upload_dir' => dirname($this->get_server_var('SCRIPT_FILENAME')) . '/files/', 'upload_url' => $this->get_full_url() . '/files/', 'user_dirs' => false, 'mkdir_mode' => 0755, 'param_name' => 'files', // Set the following option to 'POST', if your server does not support
				...
				// Defines which files can be displayed inline when downloaded:
					'inline_file_types' => '/\.(csv|xml|zip|txt)$/i', // Defines which files (based on their names) are accepted for upload:
					'accept_file_types' => '/\.(csv|xml|zip|txt)$/i', // The php.ini settings upload_max_filesize and post_max_size
		...
		protected function get_upload_path($file_name = null, $version = null) {
			...
			$eventDir = '';
			if($file_name) {
				$eventkey = $uci_admin->convert_string2hash_key($file_name);
		...
		protected function trim_file_name($name, $type = null, $index = null, $content_range = null) {
			...
			if ($file_name_arr[1] != 'zip' || $file_name_arr[1] != 'csv' || $file_name_arr[1] != 'xml' || $file_name_arr[1] != 'txt') {
				#return $file->error = 'Un Supported File Format ';
			}
		}
		...
		public function checkFileExists($fileName) {
			...
			$file_with_path = SM_UCI_IMPORT_DIR . '/' . $fileName;
