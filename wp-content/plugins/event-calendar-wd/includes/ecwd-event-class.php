    ...
    public function get_days($event) {
        if ($event->start_time !== null) {
           // echo $this->event_id.'----------'.$event->start_time.'----'.$event->end_time."<br />";
            ...
            $start_time = mktime(0, 0, 0, date('m', $event->start_time), date('d', $event->start_time), date('Y', $event->start_time));
            ...
            if ($event->multiple_day_events && ( 'MPD' == $event->day_type || 'MWD' == $event->day_type )) {
                ...
                while ($on_next_day) {
                    ...
                    if ($event->end_time > $next_day) {
                        $days[] = date('Y-m-d H:i:s', $next_day);
                    } else {
                        $on_next_day = false;
                    }
                    $next_day += 86400;
                ...
                $days[] = date('Y-m-d H:i:s', $start_time);
            }
            return $days;
        } else {
            return array();
