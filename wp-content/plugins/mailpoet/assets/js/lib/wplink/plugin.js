...
( function( $ ) {
    ...
    init: function() {
        ...
        inputs.urlType.change(function() {
            if ($(this).val().match(/^\[[a-zA-Z0-9]+\]$/)) {
                $('#link-href-field, #search-panel').hide();
                $('#wp-link-wrap').removeClass('search-panel-visible');
            } else {
                $('#link-href-field, #search-panel').show();
                $('#wp-link-wrap').addClass('search-panel-visible');
            }
            inputs.url.val($(this).val());
        });
    ...
    mceRefresh: function() {
        var e, href, newOption;

        // If link exists, select proper values.
        if ( e = editor.dom.getParent( editor.selection.getNode(), 'A' ) ) {
            // Based on href attr try to infer whether it is a shortcode or a url
            href = editor.dom.getAttrib( e, 'href' );
            newOption = inputs.urlType.find('option').filter(function(index, item) {
                return href === $(item).val();
            });

            if (newOption.length > 0) {
                inputs.urlType.val(newOption.val()).change();
            } else {
                inputs.urlType.val(inputs.urlType.find('option:first-child').val()).change();
            }
