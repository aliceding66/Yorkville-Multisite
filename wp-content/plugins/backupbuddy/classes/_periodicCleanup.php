	...
	if ( pb_backupbuddy::$options['last_backup_finish'] > 0 ) {
		...
		if ( $days_since_last > pb_backupbuddy::$options['no_new_backups_error_days'] ) {
			...
			if ( ( time() - $last_sent ) > (60*60*24) ) { // 24hrs+ elapsed since last email sent.
				$message = 'Alert! BackupBuddy is configured to notify you if no new backups have completed in `' . pb_backupbuddy::$options['no_new_backups_error_days'] . '` days. It has been `' . $days_since_last . '` days since your last completed backup.';
