	...
	public function deploy_getFile( $state, $destinationPath, $sendFile, $sendType, $finalNextStep, $seekTo = 0 ) {
		...
		*/
		...
		while( true == $keepSending ) {
			...
			// If filename contained UTF-8 chars on remote server, it came through jSON encoded and must be decoded before file is written
			if ( ! empty( $response['encoded'] ) ) {
				$saveFile = $destinationPath . utf8_decode( $sendFile );
			}
			
			// If filename contained UTF-8 chars on remote server, it came through jSON encoded and must be decoded before file is written
			if ( isset( $response['encoded'] ) && ( true === $response['encoded'] ) ) {
				$saveFile = $destinationPath . utf8_decode( $sendFile );
			}
